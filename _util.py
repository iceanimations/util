...
default_maps = [
    ...
    symlinks.symlinkMapping(
        location='\\\\dbserver\\assets',
        name='ding_dong',
        target=u'l:\\red\\ding_dong',
        stype='<SYMLINKD>'),
    symlinks.symlinkMapping(
        location='\\\\dbserver\\assets',
        name='mansour_s02',
        target=u'p:\\external\\al_mansour_season_02',
        stype='<SYMLINKD>'),
    symlinks.symlinkMapping(
        location='\\\\dbserver\\assets',
        name='mansour_s03',
        target=u'p:\\external\\al_mansour_season_03',
        stype='<SYMLINKD>'),
    symlinks.symlinkMapping(
        location='\\\\dbserver\\assets',
...
class TacticAppUtils(object):
    ...
    if _s and _user:
        ...
        def get_user_related_sobjects(self, proj, user=_user):
            ...
            return self.map_tasks_to_sobjects([
                task for task in self.all_tasks(proj, user=user)
        ...
        def sobject_to_user_task(self, sobj, user=_user):
            ...
            tasks = self.server.get_all_children(
                sobj, "sthpw/task", filters=[("assigned", str(user))])
    ...
    def checkout(self, *args, **kargs):
        ...
        snapshot = server.get_snapshot(
            args[0], context=context, version=version, versionless=versionless)
        sources = server.get_all_paths_from_snapshot(
            snapshot['code'], file_types=file_types, mode='client_repo')

        if to_sandbox_dir:
            destinations = server.get_all_paths_from_snapshot(
                snapshot['code'], file_types=file_types, mode='sandbox')
        else:
            destinations = [
                os.path.join(to_dir, os.path.basename(source))
    ...
    def all_task_processes(self, project, tasks=None):
        processes = set([
            task["process"]
            for task in (self.all_tasks(project) if not tasks else tasks)
        ])
    ...
    def cacheable(self, obj):
        ...
        return True if mi.pc.PyNode(
            obj).referenceFile() in self.get_references().keys() else False

    def date_str_to_datetime(self, string, format="%Y-%m-%d %H:%M:%S"):
        return datetime.strptime(string.split(".")[0], format)

    def get_filename_from_snap(self,
                               snap,
                               mode='sandbox',
                               translatePath=True,
                               filetype=None):
        ...
        path = self.server.get_all_paths_from_snapshot(
            snap['__search_key__'],
            file_types=[filetype] if filetype else [],
            mode=mode)[0]
        if translatePath:
    ...
    def get_project_from_search_key(self, s_key):
        ...
        if 'project' in s_key:
            prj_tag = 'project='
            return s_key[s_key.find(prj_tag) + len(prj_tag):s_key.find('&')]
        else:
            return 'sthpw'
    ...
    def get_sobject_from_task(self, task):
        ...
        if not search_type:
            return None
        return self.server.build_search_key(
            search_type[:search_type.find("?") if not-1 else len(search_type)],
            task["search_code"],
    ...
    def get_sobject_name(self, sobj):
        ...
        title = sobj_dict.get("title", sobj_dict.get("name", 'No title'))
    ...
    def get_tasks(self, project, process=None, user=None, clean=False):
        ...
        @process: take in a single process whose tasks shall be returned. None
                  implies all.
        '''
        ...
        try:
            ...
            if clean:
                pop = [
                    drat for drat in range(len(tasks))
                    if "?project=%s" % project not in str(
                        tasks[drat]["search_type"])
                ]
            ...
            filters=[('sequence_code', self.get_search_key_code(sequence)
                      if sequence else ([
                          ...
                          for seq in self.server.query(
                              'vfx/sequence',
                              filters=[('episode_code',
                                        self.get_search_key_code(episode))])
                      ] if episode else ''))] if (sequence or episode) else [])
        self.set_project(project=proj)
        return result
    ...
    def get_project_title(self, proj_code):
        result = self.server.query(
            "sthpw/project", filters=[("code", proj_code)])
        if result:
    ...
    def get_contexts_from_task(self, task):
        ...
        contexts = set([
            item['context'] for item in self.get_snapshot_from_sobject(sobj)
    ...
    def get_snapshots(self, context, task):
        ...
        for snap in snapshots:
            snapshot_dict[snap["__search_key__"]] = {
                "filename": op.basename(self.filename_from_snap(snap)),
    ...
    def get_project_snapshots(self, proj):
        return self.server.query(
            'sthpw/snapshot', filters=[('project_code', proj)])
    ...
    def get_references(
        ...
        snap_path = [
            op.normpath(
                self.get_filename_from_snap(
                    snap, mode='client_repo', filetype='maya')).lower()
    ...
    def project_code_to_title(self, proj_code):
        ...
        '''
        return self.server.query(
            "sthpw/project", [("code", proj_code)], single=True).get("title")
    ...
    def copy_snapshot(self,
        ...
        for fileEntry in server.get_all_children(
                snapshot_from['__search_key__'], 'sthpw/file'):
        ...
        for directory in dirs:
            server.add_directory(
                snapshot_to['code'],
                directory[0],
                file_type=directory[1],
                mode=mode)
        for group in groups:
            server.add_group(
                snapshot_to['code'], group[1], group[0], mode=mode)
    ...
    def get_fileobj_from_path(self, path):
        ...
        for transPath in symlinks.translatePath(
                path, linkdir=base_dir, reverse=True, single=False):
            relpath = op.relpath(transPath, base_dir)
            reldir, base_name = os.path.split(relpath)
            fileobj = self.server.query(
                'sthpw/file',
                filters=[('relative_dir', reldir.replace('\\', '/')),
                         ('file_name', base_name)],
                single=True)
            if fileobj:
                return fileobj
    ...
    def get_icon(self, obj, mode='client_repo', file_type='icon'):
        ...
        try:
            ...
            if obj.startswith('sthpw/file'):
                iconpath = self.get_file_icon(
                    obj, mode=mode, file_type=file_type)
            elif obj.startswith('sthpw/snapshot'):
                iconpath = self.get_snapshot_icon(
                    obj, mode=mode, file_type=file_type)
            elif obj.startswith('sthpw/task'):
                iconpath = self.get_task_icon(
                    obj, mode=mode, file_type=file_type)
            else:
                iconpath = self.get_sobject_icon(
                    obj, mode=mode, file_type=file_type)
    ...
    def get_sobject_icon(self,
        ...
        iconss = self.server.get_snapshot(
            sobject_skey, context=context, version=0, process=process)
        if not iconss.get('code'):
            return ''

        return self.get_snapshot_icon(
            iconss['code'], mode=mode, file_type=file_type)
    ...
    def get_task_icon(self, task, mode='client_repo', file_type='icon'):
        ...
        return self.get_sobject_icon(
            sobject_skey, mode=mode, file_type=file_type)

    def get_path_icon(self, path, mode='client_repo', file_type='icon'):
        ''' return the icon for a given path in client_repo '''
        basedir = op.normcase(
            op.normpath(self.server.get_base_dirs()['win32_client_repo_dir']))
        ...
        file_sobj = self.server.query(
            'sthpw/file',
            filters=[('relative_dir', relative_dir.replace('\\', '/')),
                     ('file_name', file_name)],
            single=True)
        if file_sobj:
            return self.get_file_icon(
                file_sobj, mode=mode, file_type=file_type)
        return ''

    def get_file_icon(self, file_sobject, mode='client_repo',
                      file_type='icon'):
        ''' get an icon from the snapshot given a file sobject '''
        sscode = file_sobject.get('snapshot_code')
        if not sscode:
            ss = self.server.query(
                'sthpw/file',
                filters=[('code', file_sobject['code'])],
    ...
    def get_snapshot_icon(self,
        ...
        filepath = self.server.get_all_paths_from_snapshot(
            snapshot_code, mode=mode, file_types=file_types)
        if filepath:
            filepath = filepath[0]
        return filepath
    ...
    def get_episode_asset(self, project, episode, asset, force_create=False):
        ...
        if force_create:
            obj = self.server.get_unique_sobject(
            ...
            obj = self.server.query(
                'vfx/asset_in_episode',
                filters=[('episode_code', episode['code']), ('asset_code',
                                                             asset['code'])],
                single=True)
    ...
    def get_sequence_asset(self, project, sequence, asset, force_create=False):
        ...
        if force_create:
            ...
            obj = self.server.query(
                'vfx/asset_in_sequence',
                filters=[('sequence_code', sequence['code']), ('asset_code',
                                                               asset['code'])],
                single=True)
    ...
    def get_shot_asset(self, project, shot, asset, force_create=False):
        ...
        if force_create:
            obj = self.server.get_unique_sobject(
                'vfx/asset_in_shot',
                data={'shot_code': shot['code'],
                      'asset_code': asset['code']})
        else:
            obj = self.server.query(
                'vfx/asset_in_shot',
                filters=[('shot_code', shot['code']), ('asset_code',
                                                       asset['code'])],
                single=True)
    ...
    prod_elem_stypes = {}
    prod_elem_stypes['vfx/episode'] = (
        'vfx/asset_in_episode', 'episode_code',
        "@SOBJECT(vfx/asset_in_episode['episode_code', '%s'].vfx/asset)")
    prod_elem_stypes['vfx/sequence'] = (
        'vfx/asset_in_sequence', 'sequence_code',
        "@SOBJECT(vfx/asset_in_sequence['sequence_code', '%s'].vfx/asset)")
    prod_elem_stypes['vfx/shot'] = (
        'vfx/asset_in_shot', 'shot_code',
    ...
    def get_production_assets(self, project, prod_elem, expand_assets=True):
        ...
        for stype, (prod_stype, code_key,
                    tel) in self.prod_elem_stypes.items():
            if search_type.startswith(stype):
                prod_assets = server.query(
                    prod_stype, filters=[(code_key, prod_elem['code'])])
                if prod_assets and expand_assets:
                    assets = {
                        a['code']: a
                        for a in server.eval(tel % prod_elem['code'])
                    }
    ...
    def is_production_asset_paired(self, prod_asset):
        prod_snapshots = self.get_snapshot_from_sobject(
            prod_asset['__search_key__'])
        rigs = [snap for snap in prod_snapshots if snap['context'] == 'rig']
        shadeds = [
            snap for snap in prod_snapshots if snap['context'] == 'shaded'
        ]
    ...
    def publish_asset_to_episode(self,
                                 project,
                                 episode,
                                 asset,
                                 snapshot,
        ...
        server.add_dependency_by_code(
            newss['code'], snapshot['code'], type='ref', tag='publish_source')
        server.add_dependency_by_code(
            snapshot['code'], newss['code'], type='ref', tag='publish_target')

        return newss

    def publish_asset_to_sequence(self,
                                  project,
                                  sequence,
                                  asset,
                                  snapshot,
    ...
    def publish_asset_to_shot(self,
                              project,
                              shot,
                              asset,
                              snapshot,
    ...
    def publish_asset(self,
                      ...
                      set_current=True):
        server = self.server
        pub_obj = self.get_production_asset(
    ...
    def get_published_snapshots_in_episode(self,
                                           project_sk,
                                           episode,
                                           asset,
                                           context=None):
        ...
        snapshots = []
        if pub_obj:
            snapshots = self.get_snapshot_from_sobject(
                pub_obj['__search_key__'])
        if context is not None:
            snapshots = [ss for ss in snapshots if ss['context'] == context]
        return snapshots

    def get_published_snapshots_in_sequence(self,
                                            project_sk,
                                            sequence,
                                            asset,
                                            context=None):
    ...
    def get_published_snapshots_in_shot(self,
                                        project_sk,
                                        shot,
                                        asset,
                                        context=None):
    ...
    def get_published_snapshots(self,
                                project_sk,
                                prod_elem,
                                asset,
                                context=None):
    ...
    def get_texture_snapshot(self,
        ...
        textures = server.get_all_children(asset['__search_key__'],
                                           'vfx/texture')
        texture_snap = {}
        if textures:
            texture_child = textures[0]
            texture_snap = server.get_snapshot(
                texture_child['__search_key__'],
        ...
        if rootContext == 'shaded':
            try:
                deps = self.get_dependencies(
                    snapshot, keyword='texture', source=False)
                texture = deps[0]
            except IndexError, KeyError:
                pass
        return texture

    def get_published_texture_snapshot(self,
                                       prod_asset,
                                       snapshot,
                                       version=0,
                                       versionless=False):
    ...
    def add_dependency(self, source, target, keyword='default'):
        ...
        server.add_dependency_by_code(
            target['code'], source['code'], type='ref', tag=source_tag)
        server.add_dependency_by_code(
            source['code'], target['code'], type='ref', tag=target_tag)
        return True

    def get_dependencies(self, snapshot, keyword='default', source=True):
        server = self.server
        source_tag, target_tag = self.get_dependency_tags(keyword)
        return server.get_dependencies(
            snapshot, tag=source_tag if source else target_tag)
    ...
    def get_published_targets_in_episode(self, snapshot, project_sk, episode):
        ...
        result = [
            target for target in targets for pub in published
            if target['code'] == pub['code']
        ]
        return result
    ...
    def get_cache_compatible_objects(self, snapshot):
        ...
        if snapshot['context'].split('/')[0] == 'rig':
            compatible = self.get_dependencies(
                snapshot, keyword='cache', source=True)
        elif snapshot['context'].split('/')[0] == 'shaded':
            compatible = self.get_dependencies(
                snapshot, keyword='cache', source=False)
        return compatible
...
for name, val in TacticAppUtils.__dict__.items():
    ...
    if isinstance(obj, type(TacticAppUtils.dummy)):
